...
allowed-tools: Read, Bash, Agent(keyword-collector), Agent(seo-title-generator)
---
...
```bash
SESSION_DIR="sessions/${ARGUMENTS}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$SESSION_DIR"
...
`{SESSION_DIR}/blog_titles.json`을 읽어서 사용자에게 보여주세요:
- 세션 폴더 경로
- 총 제목 수
- 상위 10개 샘플 제목
- 글자 수 통계 (평균, 최소, 최대)
